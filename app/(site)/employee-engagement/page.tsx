...
import { employeeEngagementPage } from "@/lib/content";

export const metadata: Metadata = {
  title: { absolute: `${employeeEngagementPage.metaTitle}` },
  description: employeeEngagementPage.metaDescription,
};
...
export default function EmployeeEngagementPage() {
  return (
    <>
      <PageHeader
        eyebrow={employeeEngagementPage.eyebrow}
        title={employeeEngagementPage.title}
        highlight={[...employeeEngagementPage.highlight]}
        intro={employeeEngagementPage.intro}
        cta={employeeEngagementPage.cta}
      />
      ...
      <section className="gutter pb-24 sm:pb-32">
        <SectionHeader
          index="01"
          eyebrow="Solutions"
          title="Our Employee Engagement Solutions"
          highlight={["Solutions"]}
        />
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {employeeEngagementPage.solutions.map((s, i) => (
      ...
      <section className="gutter py-14 sm:py-20">
        <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
          ...
          <ul>
            {employeeEngagementPage.whyChoose.map((b, i) => (
      ...
      {/* Ideal For */}
      <section className="gutter pb-24 sm:pb-32">
        <SectionHeader index="03" eyebrow="Who it's for" title="Ideal For" />
        <div className="mt-12">
          <TagCloud items={employeeEngagementPage.idealFor} />
        </div>
      </section>

      {/* How It Works */}
      <section className="gutter pb-24 sm:pb-32">
        <SectionHeader index="04" eyebrow="How it works" title="Recognise. Reward. Motivate. Perform. Retain." />
        <div className="mt-14 flex flex-wrap items-center justify-center gap-x-3 gap-y-6 sm:gap-x-5">
          {employeeEngagementPage.howItWorks.steps.map((step, i) => (
            <div key={step} className="flex items-center gap-3 sm:gap-5">
              ...
              {i < employeeEngagementPage.howItWorks.steps.length - 1 && (
        ...
        <Reveal delay={0.2}>
          <p className="mx-auto mt-10 max-w-2xl text-center text-cream-dim">
            {employeeEngagementPage.howItWorks.desc}
          </p>
        </Reveal>
      </section>
      ...
      <section className="gutter pb-24 sm:pb-32">
        ...
        <div className="mt-12 flex flex-wrap gap-3">
          {employeeEngagementPage.relatedSolutions.map((r) => (
            <Link
              key={r.label}
              href={r.href}
              data-cursor="hover"
      ...
      {/* FAQs */}
      <Faqs
        items={employeeEngagementPage.faqs}
        eyebrow="FAQs"
      ...
      <section className="gutter pb-24 sm:pb-32">
        <div className="rounded-frame border border-cream-line bg-ink-soft/30 px-7 py-14 text-center sm:px-12 sm:py-20">
          ...
          <h2 className="display mx-auto mt-5 max-w-[18ch] break-words text-[clamp(2rem,7vw,5.5rem)]">
            <RevealText text={employeeEngagementPage.finalCtaTitle} />
          </h2>
          <p className="mx-auto mt-6 max-w-xl text-cream-dim">{employeeEngagementPage.finalCtaDesc}</p>
          <div className="mt-10 flex justify-center">
            <MagneticButton href="/contact" variant="coral">
              {employeeEngagementPage.finalCtaLabel}
            </MagneticButton>
          </div>
        </div>
      </section>
