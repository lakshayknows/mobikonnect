...
import { experientialMarketingPage } from "@/lib/content";

export const metadata: Metadata = {
  title: { absolute: `${experientialMarketingPage.metaTitle}` },
  description: experientialMarketingPage.metaDescription,
};
...
export default function ExperientialMarketingPage() {
  return (
    <>
      <PageHeader
        eyebrow={experientialMarketingPage.eyebrow}
        title={experientialMarketingPage.title}
        highlight={[...experientialMarketingPage.highlight]}
        intro={experientialMarketingPage.intro}
        cta={experientialMarketingPage.cta}
      />
      ...
      <section className="gutter pb-24 sm:pb-32">
        <SectionHeader
          index="01"
          eyebrow="Solutions"
          title="Our Experiential Marketing Solutions"
          highlight={["Solutions"]}
        />
        <div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {experientialMarketingPage.solutions.map((s, i) => (
      ...
      <section className="gutter py-14 sm:py-20">
        <div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:gap-20">
          ...
          <ul>
            {experientialMarketingPage.whyChoose.map((b, i) => (
      ...
      {/* Industries We Serve */}
      <section className="gutter pb-24 sm:pb-32">
        <SectionHeader index="03" eyebrow="Industries" title="Industries We Serve" highlight={["Serve"]} />
        <div className="mt-12">
          <TagCloud items={experientialMarketingPage.industries} />
        </div>
      </section>

      {/* How It Works */}
      <section className="gutter pb-24 sm:pb-32">
        <SectionHeader index="04" eyebrow="How it works" title="Plan. Activate. Engage. Capture. Analyse." />
        <div className="mt-14 flex flex-wrap items-center justify-center gap-x-3 gap-y-6 sm:gap-x-5">
          {experientialMarketingPage.howItWorks.steps.map((step, i) => (
            <div key={step} className="flex items-center gap-3 sm:gap-5">
              ...
              {i < experientialMarketingPage.howItWorks.steps.length - 1 && (
        ...
        <Reveal delay={0.2}>
          <p className="mx-auto mt-10 max-w-2xl text-center text-cream-dim">
            {experientialMarketingPage.howItWorks.desc}
          </p>
        </Reveal>
      </section>
      ...
      {/* Related Solutions */}
      <section className="gutter pb-24 sm:pb-32">
        <SectionHeader index="05" eyebrow="Related" title="Related Solutions" highlight={["Solutions"]} />
        <div className="mt-12 flex flex-wrap gap-3">
          {experientialMarketingPage.relatedSolutions.map((r) => (
            <Link
              key={r.label}
              href={r.href}
              data-cursor="hover"
      ...
      {/* FAQs */}
      <Faqs
        items={experientialMarketingPage.faqs}
        eyebrow="FAQs"
      ...
      <section className="gutter pb-24 sm:pb-32">
        <div className="rounded-frame border border-cream-line bg-ink-soft/30 px-7 py-14 text-center sm:px-12 sm:py-20">
          ...
          <h2 className="display mx-auto mt-5 max-w-[18ch] break-words text-[clamp(2rem,7vw,5.5rem)]">
            <RevealText text={experientialMarketingPage.finalCtaTitle} />
          </h2>
          <p className="mx-auto mt-6 max-w-xl text-cream-dim">{experientialMarketingPage.finalCtaDesc}</p>
          <div className="mt-10 flex justify-center">
            <MagneticButton href="/contact" variant="coral">
              {experientialMarketingPage.finalCtaLabel}
            </MagneticButton>
          </div>
        </div>
      </section>
