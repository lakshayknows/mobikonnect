...
export function MediaPicker({
  ...
  async function handleUpload(file: File) {
    ...
    try {
      ...
      else if (result.url) onSelect(result.url);
    } catch {
    ...
    >
      <div className="flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden rounded-card border border-cream-line bg-ink">
        <div className="flex items-center justify-between border-b border-cream-line px-6 py-4">
          <h2 className="display text-base text-cream">Choose an image</h2>
          <button
            onClick={onClose}
            aria-label="Close"
        ...
        <div className="border-b border-cream-line px-6 py-4">
          ...
          <p className="mt-2 text-xs text-cream-faint">JPEG, PNG, WebP, GIF or AVIF · up to 10 MB</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            ...
            <p className="py-14 text-center text-sm text-cream-faint">
              Nothing uploaded yet. Add your first image above.
            </p>
          ) : (
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
              {blobs.map((b) => (
                <button
                  ...
                  className="group overflow-hidden rounded-lg border border-cream-line transition-colors hover:border-coral"
                >
